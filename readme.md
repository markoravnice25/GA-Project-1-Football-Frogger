...
This is my first project after 2 weeks of the General Assembly software engineering immersive program (which lasts for 13 weeks in total). I had 6 days to submit the final product.
...
I also logged my progress whilst adding JavaScript each day to solidify my understanding of what I was implementing and learning along the way.
...
2) Initial Creation of grid functions:
...
- executeKeyDown() function for movement of the main player using arrow keys, who is returned to starting position if moving out of field of play or pressing a key other than the arrow keys. Movement is achieved by using a removeKovacic() function from the current square at the start of this function and addKovacic() function at the end of this function to a new square.
- collisionEnglandScore() function for a collision occurring when an English player and Kovacic occupy the same square at the same time. A goal is given to England and Kovacic is returned to the start.
...
4) cancelArrowKeys() function to prevent the screen from moving up and down when using arrow keys - although this is not needed as in the CSS we have fixed the screen to window size.
